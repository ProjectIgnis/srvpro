...
(function() {
  // 标准库
  var CLIENT_get_authorize_key, CLIENT_get_kick_reconnect_target, CLIENT_heartbeat_register, CLIENT_heartbeat_unregister, CLIENT_import_data, CLIENT_is_able_to_kick_reconnect, CLIENT_is_able_to_reconnect, CLIENT_is_banned_by_mc, CLIENT_is_player, CLIENT_kick, CLIENT_kick_reconnect, CLIENT_pre_reconnect, CLIENT_reconnect, CLIENT_reconnect_register, CLIENT_reconnect_unregister, CLIENT_send_pre_reconnect_info, CLIENT_send_reconnect_info, CLIENT_send_replays, CORES_list, Cloud_replay_ids, ROOM_all, ROOM_bad_ip, ROOM_ban_player, ROOM_clear_disconnect, ROOM_connected_ip, ROOM_find_by_game_id, ROOM_find_by_name, ROOM_find_by_pid, ROOM_find_by_port, ROOM_find_by_title, ROOM_find_or_create_ai, ROOM_find_or_create_by_name, ROOM_find_or_create_random, ROOM_player_flee, ROOM_player_get_score, ROOM_player_lose, ROOM_player_win, ROOM_players_banned, ROOM_players_oppentlist, ROOM_players_scores, ROOM_unwelcome, ROOM_validate, Room, SERVER_clear_disconnect, SOCKET_flush_data, _, addCallback, auth, badwords, ban_user, botServer, bunyan, challonge, challonge_cache, challonge_module_name, challonge_queue_callbacks, chat_color, config, crypto, date, default_config, default_data, delete_outdated_cores, dialogues, disconnect_list, dns, duel_log, e, exec, execFile, fs, geoip, get_callback, get_latest_core_path, get_memory_usage, http, http_server, https, https_server, import_datas, imported, is_requesting, j, l, len, len1, len2, lflists, list, loadJSON, load_dialogues, load_tips, log, long_resolve_cards, m, memory_usage, merge, moment, net, oldbadwords, oldconfig, olddialogues, oldduellog, oldtips, options, os, path, pgClient, pg_client, pg_query, plugin_filename, plugin_list, plugin_path, real_windbot_server_ip, rebooted, redis, redisdb, ref, ref1, refresh_challonge_cache, release_disconnect, report_to_big_brother, request, requestListener, rmdir, roomlist, sanitize, setting_change, setting_save, settings, spawn, spawnSync, spawn_windbot, tips, update_core_paths, url, users_cache, utf8, wait_room_start, wait_room_start_arena, windbot_looplimit, windbot_process, windbots, ygopro, zlib,
    splice = [].splice;
  ...
  get_latest_core_path = global.get_latest_core_path = function() {
    if (settings.live_core) {
      return CORES_list.slice(0).pop();
    }
  };

  delete_outdated_cores = global.delete_outdated_cores = function() {
    var len2, m, room;
    if (settings.live_core) {
      update_core_paths();
      list = CORES_list.slice(0);
      list.pop();
      if (list.length > 0 && ROOM_all.length > 0) {
        for (m = 0, len2 = list.length; m < len2; m++) {
          path = list[m];
          room = _.find(ROOM_all, function(room) {
            return room && path === room.core_path;
          });
          if (!room) {
            rmdir(`./ygopro/cores/${path}`, function(error) {});
          }
        }
        return update_core_paths();
      }
    }
  };
  ...
  ROOM_find_by_game_id = global.ROOM_find_by_game_id = function(id) {
    return _.find(ROOM_all, function(room) {
      return room && room.game_id === id;
    });
  };
  ...
  Room = class Room {
    constructor(info) {
      this.name = sanitize(info.name);
      this.pass = info.pass;
      this.notes = sanitize(info.notes);
      this.hostinfo = {};
